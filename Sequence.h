...
/**
 * Class: SequenceNode
 * Class for the nodes since Sequence is a doubly-linked class. Two pointers for each node
 * (one pointing to the next node in Sequence and the other pointing to the previous node in Sequence)
 */
class SequenceNode {
    // public variables
    public: 
...
// private variables
private: 
size_t numElts;                     // variable to keep track of how many nodes are currently in linked list
SequenceNode* headPointer;          // variable to point to the first node
SequenceNode* tailPointer;          // variable to point to the last node
